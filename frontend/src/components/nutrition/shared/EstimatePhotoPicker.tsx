...
import { MAX_ESTIMATE_PHOTOS } from "../../../lib/foodEstimate";
import { prepareMealPhotoBlob, withTimeout } from "../../../lib/mealPhoto";
import { MealPhotoView } from "./MealPhotoView";

export interface EstimatePhotoItem {
  path: string;
  displayUrl: string;
}

interface EstimatePhotoPickerProps {
  photos: EstimatePhotoItem[];
  onChange: (photos: EstimatePhotoItem[]) => void;
}

const UPLOAD_TIMEOUT_MS = 30_000;
const PROCESS_TIMEOUT_MS = 60_000;

export function EstimatePhotoPicker({ photos, onChange }: EstimatePhotoPickerProps) {
  ...
  const atLimit = photos.length >= MAX_ESTIMATE_PHOTOS;
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    try {
      ...
      onChange([...photos, { path, displayUrl: path }]);
    } catch (err) {
        ...
        <div className="mb-3 grid grid-cols-2 gap-2">
          {photos.map((photo, index) => (
            <div key={`${photo.path}-${index}`} className="relative">
              <MealPhotoView src={photo.displayUrl} alt={`Estimate photo ${index + 1}`} />
              <button
                type="button"
                disabled={isLoading}
